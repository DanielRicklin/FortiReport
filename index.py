...
parser.add_argument("--port", help="set the port you want to access", default=443)
parser.add_argument(
    "--args-mode", action="store_true", help="to run the program only with arguments"
)
parser.add_argument(
    "--ssh",
    action="store_true",
    help="if you want to request your Fortigate through SSH",
)
parser.add_argument(
    "--bastion",
    action="store_true",
    help="Only for SSH if you need to access you Fortigate through a bastion",
)
parser.add_argument(
    "--api",
    action="store_true",
    help="if you want to request your Fortigate through the API",
)
parser.add_argument("--api-key", help="set the API KEY")
parser.add_argument(
    "--proxy-port",
    help="set the proxy port is you need to access you Fortigate through a proxy",
)
...
parser.add_argument(
    "--verbose", action="store_true", help="Verbose logging mode, will log all actions"
)
...
if args.args_mode:
    if args.api:
        ...
        forti = FortiGateApi(
            ...
            proxy_local_port=args.proxy_port,
    ...
    if args.ssh:
        ...
        forti = FortiGateApi(
            ip=ip, api_key=api_key, proxy_local_port=proxy_port, filename=filename
        )
        forti.get_xlsx_file()
    else:
